...
export function Performance({ arena }: { arena: ArenaState }) {
  ...
  return <div className="metrics-grid">
    ...
    <div className="metric"><div className="eyebrow">Maximum drawdown <span>↘</span></div><strong>{a.max_drawdown_pct.toFixed(2)}<small>%</small></strong><p><span className="risk-track"><i style={{ width: `${Math.min(100, a.max_drawdown_pct / (arena.settings.max_drawdown * 100) * 100)}%` }}/></span> {(arena.settings.max_drawdown * 100).toFixed(0)}% stop</p></div>
  </div>;
}
  ...
  return <>
    ...
    <div className="overview-grid">
      ...
      <section className="panel agent-panel">
        ...
        <div className="agent-status"><span className="eyebrow">CURRENT STATE</span><strong>{done ? 'Experiment concluded' : arena.agent_halted ? (arena.agent.positions.length ? 'Agent stopped · closing positions' : 'Agent stopped · holding cash') : snap.tick < 60 ? 'Learning the market rhythm' : arena.status === 'paused' ? 'Market paused' : arena.status === 'settling' ? 'Settling the account' : 'Watching for an edge'}</strong><p>{latest?.reason ?? 'Reading prices, liquidity and order flow. No privileged view of other traders.'}</p></div>
    ...
    <div className="lower-grid">
      <section className="panel positions-panel"><div className="panel-heading"><div><h2>Agent positions</h2><p>Actual holdings in the shared exchange.</p></div><span className="pill">{a.positions.length} open</span></div><div className="table-scroll"><table><thead><tr><th>Market</th><th>Shares</th><th>Average</th><th>Mark</th><th className="align-right">Unrealized P&amp;L</th></tr></thead><tbody>
        ...
        {!a.positions.length && <tr><td colSpan={5}><div className="empty-inline">{done || arena.agent_halted ? 'No open positions. The account is holding cash.' : 'All cash. Atlas will enter when a forecast clears its cost and risk limits.'}</div></td></tr>}
