...
export default function EquityChart({ points }: { points: CurvePoint[] }) {
  ...
  return <div className="equity-chart">
    <div className="panel-heading">
      <div><h2>Capital performance</h2><p>Every fill changes the result.</p></div>
    ...
    <svg viewBox="0 0 900 255" role="img" aria-label="Agent equity compared with buy-and-hold and cash" onPointerLeave={() => setHover(null)} onPointerMove={event => {
      ...
      <defs><linearGradient id="equity-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#52d6b0" stopOpacity=".14"/><stop offset="100%" stopColor="#52d6b0" stopOpacity="0"/></linearGradient></defs>
      {[0, 1, 2, 3, 4].map(i => {
      ...
      {hover != null && <g><line x1={x(active.tick)} x2={x(active.tick)} y1="20" y2="220" className="cursor-line"/><circle cx={x(active.tick)} cy={y(active.agent)} r="4" fill="#52d6b0" stroke="#101719" strokeWidth="2"/></g>}
    </svg>
