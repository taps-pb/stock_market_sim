...
export default function EquityChart({ points }: { points: CurvePoint[] }) {
  ...
  return <div className="equity-chart">
    ...
    <svg viewBox="0 0 900 255" role="img" aria-label="Agent equity compared with buy-and-hold and cash" onPointerLeave={() => setHover(null)} onPointerMove={event => {
      ...
      <defs><linearGradient id="equity-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" style={{ stopColor: 'var(--accent)' }} stopOpacity=".18"/><stop offset="100%" style={{ stopColor: 'var(--accent)' }} stopOpacity="0"/></linearGradient></defs>
      {[0, 1, 2, 3, 4].map(i => {
      ...
      {hover != null && <g><line x1={x(active.tick)} x2={x(active.tick)} y1="20" y2="220" className="cursor-line"/><circle cx={x(active.tick)} cy={y(active.agent)} r="4" style={{ fill: 'var(--accent)', stroke: 'var(--panel)' }} strokeWidth="2"/></g>}
    </svg>
