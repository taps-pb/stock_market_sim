...
export default function App() {
  ...
  const { snap, connected, setSnap, setConnected, setPortfolio } = useStore();
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-experiment"><div className="eyebrow">CURRENT EXPERIMENT <span>↗</span></div><strong>{arena ? money(arena.settings.capital) : '—'}</strong><p>Atlas + buy-and-hold<br/>Seed {arena?.settings.seed ?? '—'} · <span className="capitalize">{arena?.settings.scenario ?? 'balanced'}</span></p><button className="button primary full-width" onClick={() => setSetup(true)}>+ New experiment</button></div>
      <div className="sidebar-bottom"><span className={`connection-dot ${connected ? 'online' : ''}`}/><div>{connected ? 'Exchange connected' : 'Connecting to exchange'}<small>All funds and companies are simulated</small></div></div>
    </aside>
    <div className="main-shell">
      <header className="topbar"><div className="breadcrumb">Workspace <span>/</span> <strong>{activeView?.[1]}</strong></div><div className="topbar-controls">
        <span className={`status-label ${done ? 'finished' : ''}`}><i/>{!connected ? 'OFFLINE' : arena?.status === 'running' ? 'MARKET LIVE' : (arena?.status ?? 'LOADING').toUpperCase()}</span>
      ...
      <main className="workspace-content">
        ...
        {view === 'arena' && <ArenaView onControl={act}/>}
        {view === 'participants' && <Groups/>}
      ...
      <footer className="workspace-footer"><span><i className="dot teal"/> MARKET LAB <b>/</b> AUTONOMOUS TRADING EXPERIMENTS</span><span>Simulated market · No real funds</span></footer>
    </div>
